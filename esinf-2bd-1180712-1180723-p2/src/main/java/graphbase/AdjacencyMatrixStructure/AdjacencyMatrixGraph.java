 ...
 * @author DEI-ESINF
 */
public class AdjacencyMatrixGraph<Pais, Double> implements BasicGraph<Pais, Double>, Cloneable {
    ...
    public int numEdges;
    public ArrayList<Pais> vertices;
     ...
     */
    public Double privateGet(int x, int y) {
    ...
    public AdjacencyMatrixGraph(int initialSize) {
        ...
        edgeMatrix = (Double[][]) new Object[initialSize][initialSize];
    }
     ...
     */
    public Iterable<Pais> directConnections(Pais vertex) {
        int index = toIndex(vertex);
        if (index == -1) {
            return null;
        }
        ArrayList<Pais> connections = new ArrayList<>();
        for (int i = 0; i < numVertices; i++) {
            if (edgeMatrix[index][i] != null) {
     ...
     * in the graph
     */
    public Iterable<Double> outgoingEdges(Pais vertex) {
        int index = toIndex(vertex);
        if (index == -1) {
            return null;
        }
     ...
     * exist in the graph
     */
    public Iterable<Double> incomingEdges(Pais vertex) {
    ...
    @SuppressWarnings("unchecked")
    public Object clone() {
        AdjacencyMatrixGraph<Pais, Double> newObject = new AdjacencyMatrixGraph<Pais,Double>();

        newObject.vertices = (ArrayList<Pais>) vertices.clone();

        newObject.numVertices = numVertices;

        newObject.edgeMatrix = (Double[][]) new Object[edgeMatrix.length][edgeMatrix.length];
